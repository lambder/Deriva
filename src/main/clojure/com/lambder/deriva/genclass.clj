...
(ns com.lambder.deriva.genclass
...
(defn- generate-class [options-map]
  ...
  (let [default-options {:prefix "-" :load-impl-ns true :impl-ns (ns-name *ns*)}
        ...
        (fn [name pclasses rclass as-static is-varargs method-signature else-gen]
          (let [mname (str name)
...
;        (. gen (invokeStatic class-type (. Method (getMethod "Class forName(String)"))))
    ...
    (doseq [[pclasses super-pclasses] ctor-sig-map]
      (let [is-varargs (:varargs (meta pclasses))
            constructor-annotations (meta pclasses)
            pclasses (map the-class pclasses)
    ...
    (let [mm (non-private-methods super)]
      (doseq [^java.lang.reflect.Method meth (vals mm)]
             (emit-forwarding-method (.getName meth) (.getParameterTypes meth) (.getReturnType meth) false (.isVarArgs meth) (invoke-private-method meth "getGenericSignature")
      ...
      (@#'clojure.core/reduce1 (fn [mm ^java.lang.reflect.Method meth]
       ...
       (doseq [[mname pclasses rclass :as msig] methods]
         (emit-forwarding-method mname pclasses rclass (:static (meta msig)) (:varargs (meta msig)) (:signature (meta msig))
                                 emit-unsupported))
...
(defn- generate-interface
  ...
  (let [iname (.replace (str name) "." "/")
    ...
    (@#'clojure.core/add-annotations cv (meta name))
      ...
      (let [mv (. cv visitMethod (+ Opcodes/ACC_PUBLIC Opcodes/ACC_ABSTRACT)
                  ...
                  (Type/getMethodDescriptor (asm-type rclass)
                                            (if pclasses
                                              ...
                                              (make-array Type 0)))
                  nil nil)]
        (@#'clojure.core/add-annotations mv (meta mname))
        (dotimes [i (count pmetas)]
          (@#'clojure.core/add-annotations mv (nth pmetas i) i))
        (. mv visitEnd)))
    (. cv visitEnd)
    [iname (. cv toByteArray)]))

#_(defmacro gen-interface2
